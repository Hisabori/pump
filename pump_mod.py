...
def find_combined_signals(exchange, symbol):
    ...
    if buy_signal:
        return f"매수 신호 감지: {symbol}, RSI {last_row['RSI']:.2f}, MACD 상승 교차, 볼밴 하단 근처", analysis
    elif sell_signal:
        return f"매도 신호 감지: {symbol}, RSI {last_row['RSI']:.2f}, MACD 하락 교차, 볼밴 상단 근처", analysis
    else:
        return f"관망 중: {symbol}, RSI {last_row['RSI']:.2f}", analysis
